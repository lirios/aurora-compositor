...
public:
    ...
    explicit DrmConnectorMode(DrmConnector *connector, drmModeModeInfo nativeMode, Flags additionalFlags);

    QSize size() const;
    quint32 refreshRate() const;
    Flags flags() const;
...
{
public:
    explicit DrmConnector(DrmDevice *device, uint32_t connectorId);
    ...
    static DrmContentType convertToDrmContentType(Output::ContentType type);
    static Output::Transform convertToTransform(PanelOrientation orientation);
...
private:
    ...
    friend QDebug &operator<<(QDebug &s, const Aurora::Platform::DrmConnector *obj);
};
