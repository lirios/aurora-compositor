...
WaylandQuickItem::WaylandQuickItem(QQuickItem *parent)
    : WaylandQuickItem(*new WaylandQuickItemPrivate(), parent)
{
}
...
void WaylandQuickItem::takeFocus(WaylandSeat *device)
{
    ...
    if (surface()->client()->textInputProtocols().testFlag(WaylandClient::TextInputProtocol::TextInputV3)) {
        WaylandTextInputV3 *textInputV3 = WaylandTextInputV3::findIn(target);
        if (textInputV3)
...
#include "moc_aurorawaylandquickitem.cpp"
