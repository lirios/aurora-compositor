...
{
    const double determinant = m(0, 0) * (m(1, 1) * m(2, 2) - m(2, 1) * m(1, 2))
...
static QMatrix3x3 matrixFromColumns(const QVector3D &first, const QVector3D &second,
...
{
    return QVector3D(mat(0, 0) * v.x() + mat(0, 1) * v.y() + mat(0, 2) * v.z(),
...
{
    xyz /= xyz.y();
    return QVector2D(xyz.x() / (xyz.x() + xyz.y() + xyz.z()),
                     xyz.y() / (xyz.x() + xyz.y() + xyz.z()));
}

QMatrix3x3 Colorimetry::chromaticAdaptationMatrix(QVector2D sourceWhitepoint,
                                                  QVector2D destinationWhitepoint)
{
    ...
    const QVector3D factors =
            (bradford * xyToXYZ(destinationWhitepoint)) / (bradford * xyToXYZ(sourceWhitepoint));
...
QMatrix3x3 Colorimetry::calculateToXYZMatrix(QVector3D red, QVector3D green, QVector3D blue,
                                             QVector3D white)
{
    const auto component_scale = inverse(matrixFromColumns(red, green, blue)) * white;
    return matrixFromColumns(red * component_scale.x(), green * component_scale.y(),
...
const ColorDescription ColorDescription::sRGB = ColorDescription(
        NamedColorimetry::BT709, NamedTransferFunction::gamma22, 100, 0, 100, 100, 0);
...
ColorDescription::ColorDescription(const Colorimetry &colorimety, NamedTransferFunction tf,
                                   double sdrBrightness, double minHdrBrightness,
                                   double maxFrameAverageBrightness,
                                   double maxHdrHighlightBrightness, double sdrGamutWideness)
...
ColorDescription::ColorDescription(NamedColorimetry colorimetry, NamedTransferFunction tf,
                                   double sdrBrightness, double minHdrBrightness,
                                   double maxFrameAverageBrightness,
                                   double maxHdrHighlightBrightness, double sdrGamutWideness)
...
{
    return QVector3D(std::clamp(vect.x(), min, max), std::clamp(vect.y(), min, max),
                     std::clamp(vect.z(), min, max));
}

QVector3D ColorDescription::encodedToNits(const QVector3D &nits, NamedTransferFunction tf,
                                          double sdrBrightness)
{
    switch (tf) {
    case NamedTransferFunction::sRGB:
        return sdrBrightness
                * QVector3D(srgbToLinear(nits.x()), srgbToLinear(nits.y()), srgbToLinear(nits.z()));
    case NamedTransferFunction::gamma22:
        return sdrBrightness
                * QVector3D(std::pow(nits.x(), 2.2), std::pow(nits.y(), 2.2),
                            std::pow(nits.z(), 2.2));
...
QVector3D ColorDescription::nitsToEncoded(const QVector3D &rgb, NamedTransferFunction tf,
                                          double sdrBrightness)
{
    switch (tf) {
    case NamedTransferFunction::sRGB: {
        const auto clamped = clamp(rgb / sdrBrightness);
        return QVector3D(linearToSRGB(clamped.x()), linearToSRGB(clamped.y()),
                         linearToSRGB(clamped.z()));
    }
    case NamedTransferFunction::gamma22: {
        const auto clamped = clamp(rgb / sdrBrightness);
        return QVector3D(std::pow(clamped.x(), 1 / 2.2), std::pow(clamped.y(), 1 / 2.2),
                         std::pow(clamped.z(), 1 / 2.2));
    }
    case NamedTransferFunction::linear:
        return rgb;
    case NamedTransferFunction::scRGB:
